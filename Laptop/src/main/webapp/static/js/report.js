...
var charts = {
    ...
    show_details(data){
    	...
    	data.forEach(item => {
			...
			var sum =  formatter.format(Math.round(item.sum*100)/100);
			var min =  formatter.format(Math.round(100*item.min)/100);
			var max =  formatter.format(Math.round(100*item.max)/100);
			var avg =  formatter.format(Math.round(100*item.avg)/100);
